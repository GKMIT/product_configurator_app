	...
	var reqGet = http.request(options, function(response) {
		...
		response.on('end',function (){
			...
			if(data.statusCode == 200){
				...
				res.render('users/view_quote', {username: req.session.member_username, priv: req.session.priv, bid:'active', rfq: data.rfq, rfq_lines: data.rfq_lines, product_types: data.product_types, complexities: data.complexities });
			} else {
				res.send(data.success);
			}
		});
